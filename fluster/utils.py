...
def download(
    ...
    If mirror is provided, the mirror URL is tried once first and falls back
    to the original source on network failure.

    *filename* overrides the on-disk name, letting callers that keep a flat
    cache dir avoid collisions between URLs sharing a basename."""
    ...
    # max_retries=0 (CLI -r 0) would otherwise skip the loop entirely and
    # return without downloading, failing later with a confusing ENOENT.
...
class DownloadManager:
    ...
    def __init__(
        ...
        # Cache keys stay the original URL, so a mirror hit and a fallback hit
        ...
        # url -> the checksum its cached file was verified against, so a later
        # get() with a real checksum can't reuse a file accepted under __skip__.
    ...
    def get(self, url: str, source_checksum: str) -> str:
        ...
        while True:
            with self._lock:
                ...
                if url in self._cache and os.path.exists(self._cache[url]):
                    if source_checksum in (self._verified.get(url), "__skip__"):
                        _locked_print(f"\tReusing cached download for {filename_from_url(url)}")
                        return self._cache[url]
                    # Verified under a different (or no) checksum: fall through
                    # and re-check rather than trusting the earlier acceptance.
                    self._cache.pop(url, None)
                    self._verified.pop(url, None)
        ...
        try:
            ...
            with self._lock:
                ...
                self._verified[url] = source_checksum
                self._errors.pop(url, None)
    ...
    def _do_download(self, url: str, source_checksum: str) -> Tuple[str, bool]:
        ...
        # The cache dir is flat, so key the filename by a digest of the full
        # URL: distinct sources sharing a basename (e.g. the three bit_stream.gz
        # in MPEG2_VIDEO-MAIN) would otherwise overwrite each other. The real
        # extension is kept as a suffix so is_extractable() still works.
        cache_name = f"{hashlib.sha256(url.encode()).hexdigest()[:16]}-{filename_from_url(url)}"
        dest_path = os.path.join(self._cache_dir, cache_name)
        ...
        if skip:
            ...
            with self._download_slots:
                ...
                download(url, self._cache_dir, max_retries=self._retries, mirror=self._mirror, filename=cache_name)
